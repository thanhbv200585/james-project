...
import org.apache.james.imap.api.crowdsec.ConnectionCheck;
...
public class CrowdsecImapConnectionCheck implements ConnectionCheck {
    ...
    public Publisher<Void> validate(InetSocketAddress remoteAddress) {
        ...
        return client.getCrowdsecDecisions()
            .filter(decision -> decision.stream().anyMatch(d -> isBanned(d, ip)))
            .map(decision -> {
                throw new RuntimeException("Ip " + ip + " is not allowed to connect to IMAP server");
            });
    }
